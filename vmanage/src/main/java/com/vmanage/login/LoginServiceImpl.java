...
public class LoginServiceImpl implements LoginService {
	...
	public String verifyOtp(String otp) {
		...
		for (String otpCheck : otps) {
			System.out.println(otpCheck + " " + otp);

			if (otpCheck != null && !"".equals(otpCheck)) {
				if (otpCheck.equalsIgnoreCase(otp)) {

					System.out.println("OTP IS FOUND.");
				}
			} else {
		...
		/*
		 * List<VendorEntity> list = this.repository.findAll(); for (VendorEntity
		 * vendorEntity : list) { System.out.println(vendorEntity.getOtp() + " " + otp);
		 * if (vendorEntity.getOtp() != null && !"".equals(vendorEntity.getOtp())) { if
		 * (vendorEntity.getOtp().equals(otp)) {
		 * System.out.println("otp match to database."); } } else {
